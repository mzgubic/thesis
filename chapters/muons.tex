...
Ultimately the results were not used in the $\hmumu$ analysis
...
on ID and MS $\pt$ can be assessed more accurately, can a statistical
combination improve measurement of the CB momentum over the combined re-fit?
...
\begin{figure}[h!]
  ...
  %\includegraphics[width=0.48\textwidth]{figures/muons/vader-dimuon-CB}
  ...
  panel shows the ratio of the two distributions.}
...
ATLAS event model, but they are poorly modelled. More precisely, the
...
small), making $\alpha$ larger than 1. The goal, therefore, is to estimate
$\alpha$ on muon-by-muon basis and provide a correction to the estimate
of the uncertainty. This is done using a gradient boosting regression model
using the same datasets and variables as for the first model described in
the text above.

However, the loss function is different. In order to estimate $\alpha$,
the 84$^\text{th}$ (68\% + one tail) quantile of the distribution in Eq.
\ref{eq:muon:pull} is estimated. A quantile loss function, a
generalisation of the least absolute deviation, is used \cite{qreg}. 
