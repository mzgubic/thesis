...
The tracker is built of several subcomponents, shown in Figure
\ref{fig:exp:tracker}:
\begin{itemize}
\item Pixel detector is the innermost part of the tracker system,
and consists of three layers of silicon pixel detectors in the barrel
and endcap regions with a total of approximately 80 million channels.
Before LHC Run 2 a new layer, called insertable B-layer (IBL), was
added between the beam pipe and the then innermost pixel layer
in order to provide robustness against inefficiencies in the
pixels caused by radiation damage and improve resolution. The
resolution in the bending plane, relevant for the measurement of
$\pt$, is about $10\mu m$ \cite{CERN-LHCC-97-016, Capeans:1291633}. 
\item SCT
\item TRT
\end{itemize}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=1\textwidth]{figures/experiment/tracker}
  ...
  the silicon strip detector, called the SCT, and a transition
  radiation tracker, called TRT.
