...
binned maximum-likelihood fit, performed simutlaneously in the
twelve analysis categories. The fit is performed in the 120--130 \GeV~
region, with the bin size of 0.1 \GeV. Confidence intervals are
...
uncertainties with constraint terms. 

The observed data, representing the observed events binned in
the invariant mass, is denoted by
$\mathcal{D} = \{n_{cb}\}$, where $n_{cb}$ is the number of
data events in bin $b$ of category $c$.
%The total number of data events
%in category $c$ is denoted by $n_c = \sum_{b \in \text{bins}} n_{cb}$.
...
obtained from previous independent measurements \footnote{For practical reasons
...
The results of the maximum likelihood fit are shown in Figures
